...
st.write("Log, edit, and audit company operational overhead with local state retention, secure export, and full snapshot restoration.")
st.write("---")
...
# ==========================================
# SIDEBAR: DISASTER RECOVERY IMPORT PORT
# ==========================================
st.sidebar.header("🛠️ Database Tools")
st.sidebar.write("If the cloud server resets and your dashboard looks empty, use this port to restore your data.")

with st.sidebar.expander("🔄 Upload Saved Backup"):
    uploaded_file = st.file_uploader("Choose your latest backup file (.csv):", type=["csv"], key="recovery_uploader")
    if uploaded_file is not None:
        try:
            # Parse the uploaded backup
            restored_df = pd.read_csv(uploaded_file)
            # Standardize structural columns to prevent mismatches
            restored_df["Log ID"] = restored_df["Log ID"].astype(str)
            restored_df["Amount (₹)"] = restored_df["Amount (₹)"].astype(float)
            
            if st.button("Overwrite Server with Backup", type="primary", use_container_width=True):
                st.session_state.expense_df = restored_df
                save_all_data(restored_df)
                st.success("✓ Database successfully restored!")
                st.rerun()
        except Exception as e:
            st.error("Invalid file format. Make sure you are uploading a file generated by this app.")
